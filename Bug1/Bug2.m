...
%
% This is an updated version of Bug2 by G. Fainekos (2018.09.08)
% Bug fixes:
%   1) Fixed m-line computation when m-line is parallel to y axis
%   2) Fixed leave point determination when goal is surrounded by obstacle
%  
        ...
        function n = next(bug, robot)
            ...
            if bug.step == 2
                ...
                if abs( [robot' 1]*bug.mline') <= 0.5
                    % Am I going to hit the obstacle at the next step?
                    d = bug.goal-robot;
                    if abs(d(1)) > abs(d(2))
                        % line slope less than 45 deg
                        dx = sign(d(1));
                    ...
                    % if no obstacle at the next step follow the m-line
                    if ~bug.isoccupied(robot + [dx; dy])
